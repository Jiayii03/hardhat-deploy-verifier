...
// File @openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol@v5.2.0
...
// File @openzeppelin/contracts/interfaces/IERC4626.sol@v5.2.0
...
// File @openzeppelin/contracts-upgradeable/token/ERC20/extensions/ERC4626Upgradeable.sol@v5.2.0
...
// File @openzeppelin/contracts-upgradeable/utils/ReentrancyGuardUpgradeable.sol@v5.2.0
...
// File contracts/core/VirtualVault.sol
...
// File contracts/core/CombinedVault.sol
    ...
    OwnableUpgradeable 
{
    ...
    event Harvested(uint256 timestamp, uint256 totalAssets, uint256 oldRate, uint256 newRate);
     ...
     * Can only be called by the current owner.
     */
    function transferOwnership(address newOwner) public override(OwnableUpgradeable) {
    ...
    function addActiveProtocol(uint256 protocolId) external override onlyOwner {
        ...
        require(registry.hasAdapter(protocolId, address(_asset)), "No adapter for this asset");
     ...
     * @param protocolId ID of the protocol to remove
     */
    function removeActiveProtocol(uint256 protocolId) external override onlyOwner {
     ...
     * @param newProtocolId ID of the new protocol
     */
    function replaceActiveProtocol(uint256 oldProtocolId, uint256 newProtocolId) external override onlyOwner {
        // Check if the new protocol is registered in the registry
        require(bytes(registry.getProtocolName(newProtocolId)).length > 0, "New protocol not registered");
        require(registry.hasAdapter(newProtocolId, address(_asset)), "No adapter for new protocol");
     ...
     * @dev Check and harvest yield from all protocols
     */
    function accrueAndFlush() external override onlyOwnerOrAuthorized returns (uint256 harvestedAmount) {
        ...
        emit Harvested(block.timestamp, totalAssets, previousRedemptionRate, redemptionRate);
        return totalAssets;    
    }
     ...
     * @param amount Amount to supply
     */
    function supplyToProtocol(uint256 protocolId, uint256 amount) external override onlyOwnerOrAuthorized {
    ...
    function balanceOf(address account) public view override(ERC20Upgradeable, IERC20, IVault) returns (uint256) {
    ...
    function _distributeAssets() internal {
        ...
        for (uint i = 0; i < activeProtocolIds.length; i++) {
            ...
            uint256 supplied = adapter.supply(address(_asset), amountPerProtocol);
        }
    }
    ...
    function _withdrawFromProtocols(uint256 amount, address user) internal returns (uint256) {
        ...
        for (uint i = 0; i < activeProtocolIds.length; i++) {
            ...
            // Calculate how much we can withdraw based on receipt token balance
            uint256 withdrawAmount = receiptTokenBalance < amountPerProtocol ? receiptTokenBalance : amountPerProtocol;
            
            // Approve the adapter to transfer receipt tokens if needed
            IERC20(receiptToken).approve(address(adapter), withdrawAmount);
    ...
    function _harvestAllProtocols() internal returns (uint256 totalAssets) {
        ...
        for (uint i = 0; i < activeProtocolIds.length; i++) {
            ...
            if (aToken != address(0)) {
                IERC20(aToken).approve(address(adapter), IERC20(aToken).balanceOf(address(this)));
            }
